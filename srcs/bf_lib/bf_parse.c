...
/*   Updated: 2025/07/28 20:26:11 by gcros            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static ssize_t req_parse(t_brainfuck *this, const char *program, int loop_level);
static size_t _parse_(t_brainfuck *this, const char *program);
static ssize_t _parse_loop(t_brainfuck *this, const char *program, int loop_level);
...
static ssize_t _parse_loop(t_brainfuck *this, const char *program, int loop_level)
{
	...
	this->program[this->program_size] = (t_operator){.op = op_end, .value = loop_save};
	this->program_size++;
	count += 2;
	return count;
}
...
static size_t _parse_(t_brainfuck *this, const char *program)
{
	...
	if (op == op_none)
		return 1;
	if (this->program_size != 0 && op == this->program[this->program_size - 1].op)
		this->program[this->program_size - 1].value++;
	else {
		this->program[this->program_size] = (t_operator){.op = op, .value = 1};
		this->program_size++;
	}
	return 1;
}
